...
func Test_NewTwoPhase_TimeBounds_Begin_Close(t *testing.T) {
	...
	tx, err := NewTwoPhaseCommitTransaction(sop.ForReading, -1, true, bs, sr, cr, l2, mocks.NewMockTransactionLog(), nil)
	if err != nil {
	...
	// Over-1h clamp
	tx2, err := NewTwoPhaseCommitTransaction(sop.ForWriting, 2*time.Hour, false, bs, sr, mocks.NewMockRegistry(false), l2, mocks.NewMockTransactionLog(), nil)
	if err != nil {

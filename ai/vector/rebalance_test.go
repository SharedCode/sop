...
func TestRebalance(t *testing.T) {
	// Setup
	tmpDir, err := os.MkdirTemp("", "sop-ai-test-rebalance-*")
	if err != nil {
	...
	hits, err := idx.Query([]float32{20, 20}, 5, nil)
	if err != nil {
	...
	// However, if Rebalance failed to update Content, Delete might fail or leave ghosts?
	// Let's try to delete an item from the new cluster.
	if err := idx.Delete("c3-0"); err != nil {
		t.Fatalf("Delete failed: %v", err)
	}
	// Verify it's gone
	if _, err := idx.Get("c3-0"); err == nil {

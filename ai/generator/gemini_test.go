...
func TestBuildGeminiRequest_IncludesGenerationConfigAndTools(t *testing.T) {
	...
	if req.ToolConfig.FunctionCallingConfig.Mode != "ANY" {
		t.Fatalf("expected ANY function-calling mode, got %#v", req.ToolConfig.FunctionCallingConfig)
	}
	if got := req.ToolConfig.FunctionCallingConfig.AllowedFunctionNames; len(got) != 1 || got[0] != "execute_script" {
		t.Fatalf("expected allowed function names to include execute_script, got %#v", got)
	}
}

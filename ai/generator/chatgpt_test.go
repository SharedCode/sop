...
func TestBuildChatGPTResponsesRequest_UsesPreviousResponseIDAndMapsTools(t *testing.T) {
	...
	if request.Instructions != "You are a test assistant." {
...
func TestChatGPTOwnedReActLoop_RunExecutesFunctionCallsViaResponsesAPI(t *testing.T) {
	...
	if requests[1].Instructions != "Use tools when needed." {
...
func TestParseChatGPTResponsesStream_EmitsDeltasAndReturnsCompletedResponse(t *testing.T) {
	...
	if len(events) != 5 {
		t.Fatalf("expected normalized streamed assistant and tool events, got %#v", events)
	}
	if events[0].eventType != "assistant_message" || events[0].payload["source"] != "reasoning_summary" || events[0].payload["stream_state"] != "delta" || events[0].payload["text"] != "Checking the store" {
		t.Fatalf("unexpected streamed reasoning event: %#v", events)
	}
	if events[1].eventType != "assistant_message" || events[1].payload["phase"] != "commentary" || events[1].payload["stream_state"] != "delta" || events[1].payload["text"] != "Checking" {
		t.Fatalf("unexpected streamed assistant event: %#v", events)
	}
	if events[2].eventType != "tool_call" || events[2].payload["tool"] != "lookup_user" || events[2].payload["stream_state"] != "delta" || events[2].payload["arguments_delta"] != `{"name":"John"}` {
		t.Fatalf("unexpected streamed tool delta event: %#v", events)
	}
	args, ok := events[3].payload["args"].(map[string]any)
	if events[3].eventType != "tool_call" || events[3].payload["tool"] != "lookup_user" || events[3].payload["stream_state"] != "done" || events[3].payload["arguments"] != `{"name":"John"}` || !ok || args["name"] != "John" {
		t.Fatalf("unexpected streamed tool done event: %#v", events)
	}
	if events[4].eventType != "assistant_message" || events[4].payload["source"] != "reasoning_summary" || events[4].payload["stream_state"] != "done" || events[4].payload["text"] != "Checking the store before final answer." {
		t.Fatalf("unexpected streamed reasoning done event: %#v", events)
	}
}

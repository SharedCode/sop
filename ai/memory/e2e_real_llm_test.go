...
func TestActiveMemory_EndToEnd(t *testing.T) {
	...
	memStore := memory.NewStore[Event](cti.Btree, inmemory.NewBtree[string, sop.UUID](false).Btree, inmemory.NewBtree[DistanceKey, byte](false).Btree, bti.Btree, sti.Btree, inmemory.NewBtree[sop.UUID, memory.Document](false).Btree)

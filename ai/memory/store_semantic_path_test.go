...
func TestSemanticCategoryByPath_ReturnsExpectedLeaf(t *testing.T) {
	...
	if candidates[0].ID != leaf.ID {
		t.Fatalf("expected the best semantic leaf %s, got %s", leaf.ID, candidates[0].ID)
	}
}
...
func TestSemanticCategoryByPath_ReturnsSingleBestCandidate(t *testing.T) {
	...
	if len(candidates) != 1 {
		t.Fatalf("expected a single best candidate, got %d", len(candidates))
	}
	if candidates[0] == nil {
		t.Fatal("expected a non-nil semantic candidate")
	}
	if candidates[0].ID != leftLeaf.ID && candidates[0].ID != rightLeaf.ID {
		t.Fatalf("expected one of the tied leaves, got %s", candidates[0].ID)
	}
}

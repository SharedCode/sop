...
func buildExecuteScriptArgsSchema() string {
	...
	var schemaTemplate = `{"type":"object","properties":{"script":{"type":"array","description":"Ordered AST steps for multi-step store orchestration. Chain steps with result_var/input_var. For multi-store reads or uncertain schema, call list_stores first. list_stores returns grounded per-store lines with schema=... and optional relations=[...]; read schema=... for exact field names and read relations=[...] for related-store and join-field semantics. Use those returned relations=[...] entries as the source of truth. If the AST shape is ambiguous, call gettoolinfo('execute_script').","items":{"type":"object","properties":{"op":{"type":"string","enum":%s,"description":"Operation name."},"args":{"type":"object","description":"Operation arguments are op-sensitive. begin_tx expects {database?: string, mode: \"read\"|\"write\"}. open_store expects {transaction: string, name: string}. filter expects {condition: object}, for example {\"first_name\":{\"$eq\":\"John\"}} or {\"orders.total_amount\":{\"$gt\":500}}. join and join_right expect a right-side store plus an on map derived from the researched relation mapping."},"input_var":{"type":"string","description":"Optional variable name used as input for pipeline chaining."},"result_var":{"type":"string","description":"Optional variable name to store the result for later steps."}},"required":["op"]}}},"required":["script"]}`

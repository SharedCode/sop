...
func buildExecuteScriptArgsSchema() string {
	...
	scriptDesc := "Ordered AST steps for multi-step store orchestration. Chain steps with result_var/input_var. For multi-store reads or uncertain schema, call list_stores first. list_stores returns grounded per-store lines with schema=... and optional relations=[...]; read schema=... for exact field names and value types, then align those researched field names with the user's criteria values. Read relations=[...] literally: in users_orders(key->users.key), the field before -> is the target-store join field and the field after -> is the current-store field path. Use those returned relations=[...] entries as the source of truth. join and join_right emit a combined flat record by default, so continue using dotted store-qualified field paths unless a later project step reshapes the output. If the AST shape is ambiguous, call gettoolinfo('execute_script')."
	argsDesc := "Operation arguments are op-sensitive. begin_tx expects {database?: string, mode: \"read\"|\"write\"}. open_store expects {transaction: string, name: string}. filter expects {condition: object}, for example {\"first_name\":{\"$eq\":\"John\"}} or {\"orders.total_amount\":{\"$gt\":500}}. Use the researched schema field name as the predicate key and the user's criterion as the predicate value; do not emit booleans such as {\"first_name\":true}. join and join_right must reuse researched relation mappings instead of inventing field pairs."
	onDesc := "Concrete left-to-right join mapping. Keys are current or left-record field names, values are right-side field names. Reuse the confirmed relation mapping exactly, for example {\"value\":\"key\"}. If list_stores returned users_orders(key->users.key), treat key as the target-store field and users.key as the current-store field path, then translate those grounded fields into the join mapping your join step expects. Do not invent placeholder keys or ambiguous store-qualified names."
	conditionDesc := "Concrete predicate object for filter/select steps, for example {\"first_name\":{\"$eq\":\"John\"}} or {\"orders.total_amount\":{\"$gt\":500}}. Use the schema-confirmed field path as the key and the user-requested literal or comparison value as the predicate value; never use booleans or nulls as stand-ins."

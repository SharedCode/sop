...
func buildExecuteScriptArgsSchema() string {
	...
	var schemaTemplate = `{"type":"object","properties":{"script":{"type":"array","description":"Ordered AST steps for multi-step store orchestration. Chain steps with result_var/input_var. For multi-store reads or uncertain schema, call list_stores first. list_stores returns grounded per-store lines with schema=... and optional relations=[...]; read schema=... for exact field names and read relations=[...] for related-store and join-field semantics. Use those returned relations=[...] entries as the source of truth. join and join_right emit a combined flat record by default, so continue using dotted store-qualified field paths unless a later project step reshapes the output. If the AST shape is ambiguous, call gettoolinfo('execute_script').","items":{"type":"object","properties":{"op":{"type":"string","enum":%s,"description":"Operation name."},"args":{"type":"object","description":"Operation arguments are op-sensitive. begin_tx expects {database?: string, mode: \"read\"|\"write\"}. open_store expects {transaction: string, name: string}. filter expects {condition: object}, for example {\"first_name\":{\"$eq\":\"John\"}} or {\"orders.total_amount\":{\"$gt\":500}}. join and join_right must reuse researched relation mappings instead of inventing field pairs.","properties":{"database":{"type":"string","description":"Database name for steps that override the current database, such as begin_tx or open_db."},"mode":{"type":"string","enum":["read","write"],"description":"Transaction mode for begin_tx."},"transaction":{"type":"string","description":"Transaction variable produced by begin_tx and reused by open_store, commit_tx, or rollback_tx."},"name":{"type":"string","description":"Store name for open_store."},"store":{"type":"string","description":"Store name or previously opened store variable. For join/join_right this is the preferred explicit right-side store reference when the join target is already grounded."},"with":{"type":"string","description":"Legacy alias for the right-side join store or store variable."},"target":{"type":"string","description":"Relation-target store variable for relation-driven joins, for example {\"relation\":\"users_orders\",\"target\":\"orders_store\"}."},"relation":{"type":"string","description":"Confirmed relation name from list_stores output. Prefer relation+target when list_stores returned an explicit relation path."},"on":{"type":"object","description":"Concrete left-to-right join mapping. Keys are current or left-record field names, values are right-side field names. Reuse the confirmed relation mapping exactly, for example {\"value\":\"key\"}. Do not invent placeholder keys or ambiguous store-qualified names.","additionalProperties":{"type":"string"}},"type":{"type":"string","enum":["left","right"],"description":"Optional join flavor when a step needs to force left or right join semantics."},"condition":{"type":"object","description":"Concrete predicate object for filter/select steps, for example {\"first_name\":{\"$eq\":\"John\"}} or {\"orders.total_amount\":{\"$gt\":500}}."},"field":{"type":"string","description":"Single field name used by compact filter/sort forms."},"op":{"type":"string","description":"Compact operator for filter/update forms, such as ==, >, or <=."},"value":{"description":"Literal value payload for compact filter/update/return forms."},"fields":{"type":"array","description":"Projection field list used by project steps. Keep dotted store-qualified field paths after joins unless project intentionally reshapes the output.","items":{"type":"string"}},"direction":{"type":"string","enum":["asc","desc"],"description":"Scan or sort direction when supported by the step."},"limit":{"type":"integer","description":"Maximum number of records to keep for limit-style steps."}},"additionalProperties":true},"input_var":{"type":"string","description":"Optional variable name used as input for pipeline chaining."},"result_var":{"type":"string","description":"Optional variable name to store the result for later steps."}},"required":["op"]}}},"required":["script"]}`
	return fmt.Sprintf(schemaTemplate, ops)
}

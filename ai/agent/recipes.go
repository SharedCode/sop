...
func buildExplicitRecipes(taskClassification TaskContextClassification) []RecipeItem {
	...
	if hasStoresRecipeNeed(taskClassification) {
		items = append(items, RecipeItem{
			...
			Protocol:   []string{"research with list_stores first when schema, fields, types, or joins are unclear", "scope list_stores with stores:[...] when likely targets are known", "parse relations literally: target_store(target_field->current_store.field); before -> target, after -> current", "treat list_stores relations as the source of truth for related stores and join keys", "use gettoolinfo('execute_script') only when the AST shape is unclear"},
		...
		items = append(items, RecipeItem{
			...
			Protocol:   []string{"reuse researched relations before rewriting a join", "for target_store(target_field->current_store.field), keep those grounded fields in the join mapping", "replace only the malformed join slice instead of restarting the whole AST", "keep valid stores, step order, and field paths intact while repairing the join"},
		...
		items = append(items, RecipeItem{
			...
			Protocol:   []string{"use concrete predicate objects for filters", "map researched schema fields to user criteria values/operators", "preserve exact dotted field paths for joined-field predicates", "ground predicate fields and operators in researched schema before retrying"},

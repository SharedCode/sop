...
func Test_TwoTransactionsWithNoConflict(t *testing.T) {
	...
	trans2, err := incfs.NewTransaction(ctx, sop.TransactionOptions{Mode: sop.ForWriting, MaxTime: -1, Logging: false})

	trans.Begin(ctx)
	trans2.Begin(ctx)

	pk, p := newPerson("tracy", "swift", "female", "email", "phone")
	b3, err := incfs.OpenBtree[PersonKey, Person](ctx, "persondb", trans, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.Fail()
	}
	...
	b32, err := incfs.OpenBtree[PersonKey, Person](ctx, "persondb", trans2, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.Fail()
	}
...
func Test_TwoPhaseCommitRolledback(t *testing.T) {
	...
	t0, _ := incfs.NewTransaction(ctx, sop.TransactionOptions{Mode: sop.ForWriting, MaxTime: -1, Logging: false})
	t0.Begin(ctx)

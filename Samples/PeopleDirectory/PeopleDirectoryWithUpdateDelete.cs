...
namespace Sop.Samples
{
	public class PeopleDirectoryWithUpdateDelete : Sample
    {
        ...
        const int MaxCount = 30000;
        const int TransactionSize = 10000;
		...
		public void Run()
		{
            bool storeValueInDataSegment = true;

            Console.WriteLine("{0}: PeopleDirectoryWithUpdateDelete demo started...", DateTime.Now);

		    IStoreFactory sf = new StoreFactory();
            PeopleStore = sf.Get<long, Person>(Server.SystemFile.Store, "People", null, true, storeValueInDataSegment);
            PeopleStoreByName = sf.Get<PersonKey, long>(Server.SystemFile.Store, "PeopleByName", new PersonComparer(), true, storeValueInDataSegment);

			if (PeopleStore.Count == 0)
				Populate();
			else
			{
                Console.WriteLine("Processing {0} records (PeopleStore and PeopleStoreByName).", PeopleStore.Count * 2);
				ReadAll();
			}
			Server.Commit();
            Server.Dispose();
		...
		{
			int ZipCodeCtr = 5000;
            Person[] NewPeople = new Person[BatchSize];
			int NewPeopleIndex = 0;
            bool oneTimeUpdateRead = true;
			for (int i = 0; i < MaxCount; i++)
			{
                if (i > 15000 && oneTimeUpdateRead)
                {
                    oneTimeUpdateRead = false;
                    for (int i10 = 0; i10 < 14000; i10++)
                    {
                        int pid10 = i10 + 1;
		...
		void ReadAll()
		{
            ...
            if (Ctr != PeopleStore.Count)
                ...
                Console.WriteLine("Read {0} items (PeopleStore and PeopleStoreByName).", Ctr * 2);
		}

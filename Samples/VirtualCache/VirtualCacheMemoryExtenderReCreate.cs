...
namespace Sop.Samples
{
    ...
    /// NOTE: this was used for testing the SOP async IO completion thread simulating IO exceptions.
    /// </summary>
    public class VirtualCacheMemoryExtenderReCreate
    {
        private Log.Logger _logger;
        private List<VirtualCacheBase> vcs;
        ...
        public void Run()
        {
            ...
            Console.WriteLine("You just 'cached' & 'accessed all' {0} records across {1} VirtualCache running on a dedicated thread each.", MaxCacheEntries * ThreadCount, ThreadCount);
        }
        ...
        private void DisposeVCs(List<VirtualCacheBase> vcs, int targetId = -1)
        {
            if (targetId == -1)
            {

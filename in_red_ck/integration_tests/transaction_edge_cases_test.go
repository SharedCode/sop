...
func Test_TwoTransactionsUpdatesOnSameItem(t *testing.T) {
	t1, _ := in_red_ck.NewTransaction(in_red_ck.ForWriting, -1, false)
	t2, _ := in_red_ck.NewTransaction(in_red_ck.ForWriting, -1, false)
	...
	if !found {
		...
		t1, _ = in_red_ck.NewTransaction(in_red_ck.ForWriting, -1, false)
		t1.Begin()
		b3, _ = in_red_ck.OpenBtree[PersonKey, Person](ctx, "persondb77", t1)
	}

	b32, _ := in_red_ck.OpenBtree[PersonKey, Person](ctx, "persondb77", t2)
	...
	t1, _ = in_red_ck.NewTransaction(in_red_ck.ForReading, -1, false)
	t1.Begin()
...
func Test_TwoTransactionsOneReadsAnotherWritesSameItem(t *testing.T) {
	t1, _ := in_red_ck.NewTransaction(in_red_ck.ForWriting, -1, false)
	t2, _ := in_red_ck.NewTransaction(in_red_ck.ForReading, -1, false)
...
func Test_ConcurrentCommitsComplexDupeAllowed(t *testing.T) {
	...
	f3 := func() error {
		t3, _ := in_red_ck.NewTransaction(in_red_ck.ForWriting, -1, false)
		t3.Begin()

...
func Test_TwoTransactionsWithNoConflict(t *testing.T) {
	...
	trans2, err := in_red_ck.NewTransaction(sop.ForWriting, -1, false)

	trans.Begin()
	trans2.Begin()

	pk, p := newPerson("tracy", "swift", "female", "email", "phone")
	b3, err := in_red_ck.OpenBtree[PersonKey, Person](ctx, "persondb", trans, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.Fail()
	}
	...
	b32, err := in_red_ck.OpenBtree[PersonKey, Person](ctx, "persondb", trans2, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.Fail()
	}
...
func Test_TwoPhaseCommitRolledback(t *testing.T) {
	...
	if err := twoPhase.Phase1Commit(ctx); err == nil {
		...
		b3, _ = in_red_ck.OpenBtree[int, string](ctx, "twophase", t1, nil)

...
func (nr *nodeRepository) activateInactiveNodes(ctx context.Context, nodes []sop.KeyValuePair[*btree.StoreInfo, []interface{}], handles []cas.RegistryPayload[sop.Handle]) ([]cas.RegistryPayload[sop.Handle], error) {
	if len(nodes) == 0 {
		return nil, nil
	}
	// vids := nr.convertToRegistryRequestPayload(nodes)
...
func (nr *nodeRepository) touchNodes(ctx context.Context, nodes []sop.KeyValuePair[*btree.StoreInfo, []interface{}], handles []cas.RegistryPayload[sop.Handle]) ([]cas.RegistryPayload[sop.Handle], error) {
	if len(nodes) == 0 {
		return nil, nil
	}
	// vids := nr.convertToRegistryRequestPayload(nodes)

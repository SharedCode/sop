...
_manage_search = lib.manageSearch
_manage_search.argtypes = [ctypes.c_longlong, ctypes.c_int, ctypes.c_char_p, ctypes.c_char_p]
_manage_search.restype = ctypes.POINTER(ctypes.c_char)

def manage_search(ctxID: int, action: int, targetID: str, payload: str) -> str:
    tID = None
    if targetID is not None:
        tID = to_cstring(targetID)

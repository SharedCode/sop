...
func Test_StreamingDataStoreInvalidCases(t *testing.T) {
	...
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "xyz", trans, dataPath, nil)
...
func Test_StreamingDataStoreBasicUse(t *testing.T) {
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStore", trans, dataPath, nil)
...
func Test_StreamingDataStoreMultipleItems(t *testing.T) {
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStoreM", trans, dataPath, nil)
	encoder, _ := sds.Add(ctx, "fooVideo")
	for i := 0; i < 10; i++ {
...
func Test_StreamingDataStoreDeleteAnItem(t *testing.T) {
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStoreD", trans, dataPath, nil)
	encoder, _ := sds.Add(ctx, "fooVideo")
	for i := 0; i < 10; i++ {
		encoder.Encode(fmt.Sprintf("%d. a huge chunk, about 12MB.", i))
	}
	...
	trans, _ = in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ = in_red_cfs.OpenStreamingDataStore[string](ctx, "videoStoreD", trans, nil)
...
func Test_StreamingDataStoreBigDataUpdate(t *testing.T) {
	...
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStoreU", trans, dataPath, nil)
	encoder, _ := sds.Add(ctx, "fooVideo2")
	for i := 0; i < 10; i++ {
		encoder.Encode(fmt.Sprintf("%d. a huge chunk, about 10MB.", i))
	}
...
func Test_StreamingDataStoreUpdateWithCountCheck(t *testing.T) {
	...
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStore2", trans, dataPath, nil)
...
func Test_StreamingDataStoreUpdateExtend(t *testing.T) {
	...
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStore4", trans, dataPath, nil)
	encoder, _ := sds.Add(ctx, "fooVideo3")
	encodeVideo(encoder, 5)
	trans.Commit(ctx)
...
func Test_StreamingDataStoreUpdate(t *testing.T) {
	...
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStore5", trans, dataPath, nil)
	encoder, _ := sds.Add(ctx, "fooVideo")
	encodeVideo(encoder, 5)
	trans.Commit(ctx)
...
func Test_StreamingDataStoreDelete(t *testing.T) {
	...
	trans, _ := in_red_cfs.NewTransaction(sop.ForWriting, -1, true)
	trans.Begin()
	sds, _ := in_red_cfs.NewStreamingDataStore[string](ctx, "videoStore3", trans, dataPath, nil)

...
```bash
docker pull ghcr.io/sharedcode/sop-quickstart:stable
docker run --rm ghcr.io/sharedcode/sop-quickstart:stable
```

Full server from a release bundle, three commands:

```bash
curl -LO https://github.com/SharedCode/sop/releases/latest/download/sop-bundle-linux-amd64.tar.gz

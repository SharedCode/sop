...
*   **`github.com/sharedcode/joltrin/infs`**: The primary and recommended backend. It uses the local filesystem for both metadata (via a high-performance hashmap) and data. Redis is used strictly for **caching and coordination** (locking), not for data persistence.
...
*   **`github.com/sharedcode/joltrin/incfs`**: The "Hybrid" backend. It combines:

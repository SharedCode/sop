...
namespace Sop.OnDisk.Algorithm.BTree
{
    ...
    {
        private short GetIndex(BTree.BTreeAlgorithm bTree, BTreeItemOnDisk item)
        {
            short index = 0;
            if (Count > 1)
            {
            ...
            else if (Count == 1)
            {
                if (bTree.Comparer != null)
                {
                    if (bTree.ComparerWrapper.Compare(Slots[0], item) < 0)
                        index = 1;
                }
                else
                {
        ...
        {
            RemoveFromBTreeBlocksCache(bTree, node.DiskBuffer);
        }

        private void RemoveFromBTreeBlocksCache(BTree.BTreeAlgorithm bTree, Sop.DataBlock headBlock)
        {
            if (headBlock != null)
            {
                headBlock.ClearData();
                ...
                {
                    Sop.DataBlock db = headBlock;
                    while (db != null)
                    {
                        long ID = bTree.GetId(db);
                        if (ID < 0)
                            break;
                        bTree.Blocks.Remove(ID);
                        db = db.Next;
                    }
                }
            }
        ...
        protected internal void Add(BTreeAlgorithm bTree, BTreeItemOnDisk item, BTreeNodeOnDisk parent = null)
        {
            ...
            while (true)
            {
                ...
                index = currentNode.GetIndex(bTree, item);
        ...
        internal void Dispose(bool autoDisposeItem, bool forRecycling = false, bool disposeNodeShellOnly = false)
        {
            ...
            if (DiskBuffer == null && Count == 0) return;

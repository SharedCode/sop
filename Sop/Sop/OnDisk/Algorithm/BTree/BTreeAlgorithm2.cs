...
namespace Sop.OnDisk.Algorithm.BTree
{
    ...
    internal partial class BTreeAlgorithm
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        protected virtual Sop.DataBlock GetDiskBlock(LinkedListOnDisk dataSet, long id)
        {
        ...
        public void Remove()
        {
            ...
            if (temp != null)
            {
                ...
                RegisterChange(true);
                if (temp.Value != null && temp.Value.Data is IDisposable &&
                    AutoDisposeItem)
        ...
        internal void RemoveBlock(LinkedListOnDisk dataSet, Sop.DataBlock d)
        {
            if (d == null || dataSet == null) return;
            if (d.DataAddress < 0 || (RootNode.DiskBuffer == d || RootNode.GetAddress(this) == d.DataAddress)) return;
            long id = GetId(d);
            currentDataBlock = null;
            PromoteLookup.Remove(id);
            dataSet.DataBlockDriver.Remove(dataSet, d);
        }
        ...
        private bool OnMaxCapacity(BTreeNodeOnDisk node)
        {
            if (node.IsDirty)
            {
                ...
                {
                    BTreeItemOnDisk itm = node.Slots[i];
                    //** if Data is not long int... (long ints are saved with Keys)
                    if (!IsDataInKeySegment)
                    {
                        if (itm != null && itm.IsDirty)
                        {
                            if (GetId(itm.Value.DiskBuffer) == -1 || itm.Value.IsDirty)
                            {
                                ...
                                if (sortedDict != null && !sortedDict.IsDisposed)
                                    sortedDict.Flush();
                                Sop.DataBlock b = WriteToBlock(itm.Value);
        ...
        protected int EndTreeMaintenance()
        {
            if (_maintenanceCallCount > 0 && --_maintenanceCallCount == 0)
            {
                ...
                if (PromoteLookup.Count > 0)
                {
                    ...
                    {
                        if (PromoteLookup.Count >= MruManager.MaxCapacity)
                        {
        ...
        public bool Query(QueryExpression[] items, out QueryResult[] results)
        {
            ...
            bool r = false;
            int i;
            bool valueLoaded = true;
            BeginTreeMaintenance();
            for (i = 0; i < items.Length; i++)
            {
                if (items[i].Key == null) continue;
                bool valueFilterFuncDefined = items[i].ValueFilterFunc != null;
                ...
                results[i].Found = true;
                r = true;
                var key = (BTreeItemOnDisk) this.CurrentEntry;
                if (!key.ValueLoaded)
                {
                    valueLoaded = false;
                    long da = GetId(key.Value.DiskBuffer);
                    batchedIDs.Add(da, new KeyValuePair<BTreeItemOnDisk, int>(key, i));
                }
                else
                    results[i].Value = key.Value.Data;
            }
            if (!valueLoaded && batchedIDs.Count > 0)
            {
                HintSequentialRead = true;
                _sequentialReadBatchedIDs.Clear();
                _sequentialReadBatchedIDs.Add(null);
                foreach (KeyValuePair<BTreeItemOnDisk, int> value in batchedIDs.Values)
                {
                    if (!results[value.Value].Found) continue;
                    _sequentialReadBatchedIDs[0] = value.Key;
                    results[value.Value].Value = CurrentValue;
                }
                HintSequentialRead = false;
            }
            EndTreeMaintenance();
        ...
        internal void RemoveItem(BTreeItemOnDisk deletedItem)
        {
            ...
            {
                if (deletedItem.ValueLoaded)
                {
                    if (DataSet != null)
                        RemoveBlock(DataSet, deletedItem.Value.DiskBuffer);
                }
                else
                {
        ...
        private void RemoveDeletedDataBlocks()
        {
            ...
            {
                foreach (long da in _deletedDataBlocks.Keys)
                {
                    var db = new Sop.DataBlock(DataBlockSize);
                    ((DataBlockDriver) DataBlockDriver).ReadBlockFromDisk(this, da, true, db);
                    if (DataSet != null)
        ...
        private readonly Collections.Generic.SortedDictionary<long, byte> _deletedDataBlocks = new Collections.Generic.SortedDictionary<long, byte>();
        ...
        internal SortOrderType CurrentSortOrder = SortOrderType.Ascending;
        ...
        {
            get { return _comparerWrapper ?? (_comparerWrapper = new BTreeComparer(Comparer)); }
        }

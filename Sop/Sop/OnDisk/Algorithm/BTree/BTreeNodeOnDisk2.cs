...
namespace Sop.OnDisk.Algorithm.BTree
{
    ...
    internal partial class BTreeNodeOnDisk
    {
        ...
        public void Pack(IInternalPersistent parent, System.IO.BinaryWriter writer)
        {
            ...
            // study: separate Node Value region from Key region if in same region so we can optimize Unpack,
            // i.e. - delay reading of Node Item Values when code request for the Item value.
                           ...
                           System.IO.BinaryReader reader)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                CollectionOnDisk.ReadPersistentData(parent, reader, ref vn);
                Slots[i].VersionNumber = (long)vn;

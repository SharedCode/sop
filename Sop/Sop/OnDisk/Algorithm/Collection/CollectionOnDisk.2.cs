...
namespace Sop.OnDisk.Algorithm.Collection
{
    internal abstract partial class CollectionOnDisk
    {
        ...
        protected internal virtual int UpdateCount(UpdateCountType updateType)
        {
            if (updateType == UpdateCountType.Increment)
                DataBlockDriver.HeaderData.Count++;
            else
                DataBlockDriver.HeaderData.Count--;
            return DataBlockDriver.HeaderData.Count;
        }
        ...
        public object SyncRoot
        {
            get
            {
                return _syncRoot ?? (_syncRoot = new Sop.Collections.BTree.Synchronizer());
            }
            set { _syncRoot = value; }
        }
        ...
        public Transaction.ITransactionLogger Transaction
        {
            get
            {
                ...
                ////** return server root transaction if there is no transaction available on collections level...
                //if (_instanceTransaction == null && File.Server.Transaction != null)
                //    return (Transaction.ITransactionLogger) File.Server.Transaction;
                ////**

...
namespace Sop.OnDisk.Algorithm.Collection
{
    internal abstract partial class CollectionOnDisk
    {
            ...
            )
        {
            ...
            switch (t)
            {
                case PersistenceType.Custom:
                    ...
                    byte objectTypeId = reader.ReadByte();
        ...
        /// <param name="itemType"> </param>
        public static void WritePersistentData(IInternalPersistent parent,
            object objectToSerialize, BinaryWriter writer, ItemType itemType)
        {
            if (objectToSerialize != null)
            {
                ...
                if (objectToSerialize is IInternalPersistent)
                {
                    ...
                    //** save Value type so we can deserialize to the same type.
                    var objectTypeId = file.Server.TypeStore.RegisterType(objectToSerialize);
                    writer.Write(objectTypeId);
                ...
                else if (itemType == ItemType.Key && parent is BTreeAlgorithm &&
                ...
                else if (itemType == ItemType.Value && parent is BTreeAlgorithm &&

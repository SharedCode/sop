...
namespace Sop.OnDisk
{
    ...
    internal class TypeStore
    {
        ...
        public byte RegisterType(object value)
        {
            if (value is SortedDictionaryOnDisk)
                return (byte) BuiltinTypes.SortedDictionaryOnDisk;
            if (value is FileSet)
                return (byte)BuiltinTypes.FileSet;
            if (value is File.File)
                return (byte)BuiltinTypes.File;
            if (value is BTreeAlgorithm)
                return (byte)BuiltinTypes.BTreeAlgorithm;
            if (value is SharedBlockOnDiskList)
                return (byte)BuiltinTypes.SharedBlockOnDiskList;
            if (value is LinkedListOnDisk)
                return (byte)BuiltinTypes.LinkedListOnDisk;
            if (value is LinkedListOnDisk.LinkedItemOnDisk)
                return (byte)BuiltinTypes.LinkedListOnDiskItemOnDisk;
            if (value is BTreeNodeOnDisk)
                return (byte)BuiltinTypes.BTreeOnDiskTreeNode;
            if (value is ItemOnDisk)
                return (byte)BuiltinTypes.BTreeItemOnDisk;
            if (value is DeletedBlockInfo)
                return (byte)BuiltinTypes.DeletedBlockInfo;
            if (value is DataReference)
                return (byte)BuiltinTypes.DataReference;
            if (value is Transaction.Transaction.BackupDataLogKey)
                return (byte)BuiltinTypes.BackupDataLogKey;
            if (value is Transaction.Transaction.BackupDataLogValue)
                return (byte)BuiltinTypes.BackupDataLogValue;
            return (byte)BuiltinTypes.UserDefined;
        }
    }
}
...
namespace Sop.Transaction
{
    ...
    internal class TransactionRoot : TransactionBase, Sop.Transaction.ITransactionRoot
    {
        ...
        private void SetupFilenames(string filename)
        {
            ...
            _loggerFilename = string.Format("{0}{1}{2}.txt", LogFolder, System.IO.Path.DirectorySeparatorChar, filename);
            _loggerTransDetailsFilename = string.Format("{0}{3}{1}{2}.txt", LogFolder, filename, DetailsLiteral, System.IO.Path.DirectorySeparatorChar);
            _loggerTransDetailsLogFilename = string.Format("{0}{3}{1}{2}.log", LogFolder, filename, DetailsLiteral, System.IO.Path.DirectorySeparatorChar);
        ...
        private GenericLogger Logger
        {
            ...
            {
                if (_logger == null)
                    _logger = new GenericLogger(_loggerFilename);
                return _logger;
            }
        }
        ...
        private SingleLineWriter LoggerTransDetails
        {
            ...
            {
                if (_loggerTransDetails == null)
                    _loggerTransDetails = new SingleLineWriter(_loggerTransDetailsFilename);
                return _loggerTransDetails;
            }
        }
        ...
        public static TransactionRoot BeginRoot(string serverRootPath)
        {
            ...
            if (!serverRootPath.EndsWith("" + System.IO.Path.DirectorySeparatorChar))
                serverRootPath += System.IO.Path.DirectorySeparatorChar;
            ...
            using (plh)
            {
                ...
                if (Sop.Utility.Utility.FileExists(loggerFilename))
                {
                    ...
                    using (reader)
                    {
                        ...
                        {
                            plh.ProcessedLines = lineCtr;
                            if (System.IO.File.Exists(previousTokenValue))
                                System.IO.File.Delete(previousTokenValue);
                            var infFile = string.Format("{0}.{1}", previousTokenValue, OnDisk.ObjectServer.DataInfExtensionLiteral);
                            if (System.IO.File.Exists(infFile))
                                System.IO.File.Delete(infFile);
                            plh.MarkLineSuccess();
                        }
                    }
                }
        ...
        {
            string logFolder = string.Format("{0}{1}TransactionLogs", serverPath, System.IO.Path.DirectorySeparatorChar);
            if (System.IO.Directory.Exists(logFolder))
            {
                    ...
                    using (reader)
                    {
                        ...
                        else
                        {
                            ...
                            if (parts != null && parts.Length == 3)
                            {
                                ...
                                // TODO: check whether we need to create a restart log file for logging these exceptions
        ...
        private void CleanupLogger()
        {
            ...
            {
                //** remove the log file
                if (System.IO.File.Exists(_loggerFilename))
            ...
            {
                if (System.IO.File.Exists(_loggerTransDetailsFilename))
            ...
            if (_loggerTransDetailsFileStream != null)
            {
                ...
                if (System.IO.File.Exists(_loggerTransDetailsLogFilename))
        ...
        // 2. track updated segments not in Add/Growth to allow rollback of these segments.
        ...
        protected internal override void RegisterAdd(CollectionOnDisk collection, long blockAddress, int blockSize)
        {
            ...
            Transaction.RegisterAdd(_addBlocksStore, _fileGrowthStore, _recycledSegmentsStore, collection, blockAddress,
                                    blockSize, true);
        }
        ...
        #region FileGrowth, RecycledCollection, Add Segment Stores
        ...
        private Collections.Generic.ISortedDictionary<Transaction.RecordKey, long> _addBlocksStore =
        ...
        private Collections.Generic.ISortedDictionary<Transaction.RecordKey, long> _recycledBlocksStore = 
            new Collections.Generic.ConcurrentSortedDictionary<Transaction.RecordKey, long>(new Transaction.RecordKeyComparer<Transaction.RecordKey>());

        private Collections.Generic.ISortedDictionary<Transaction.RecordKey, long> _recycledSegmentsStore =
            ...
            ConcurrentIOPoolManager readPool, ConcurrentIOPoolManager writePool)
        {
            ...
            foreach (KeyValuePair<long, int> area2 in region)
            {
                ...
                Region region2 = RegionLogic.RemoveIntersections(_recycledSegmentsStore, key,
                ...
                if (region2 != null && region.Count > 0)
                {
                    foreach (KeyValuePair<long, int> area3 in region2)
                    {
                        ...
                        Region region3 = RegionLogic.RemoveIntersections(_addBlocksStore, key,
        ...
        protected internal override void RegisterRecycleCollection(CollectionOnDisk collection, long blockAddress,
                                                                   int blockSize)
        {
            Transaction.RegisterFileGrowth(_recycledSegmentsStore, collection, blockAddress, blockSize, true);
        }

...
func AddToBreakNodeThenRemoveAll(t *testing.T) {
	...
	trans.Begin(ctx)
	b3, _ = infs.OpenBtree[PersonKey, Person](ctx, "personfoo", trans, Compare)
...
func Test_AddThenTripleUpdatesPerson(t *testing.T) {
	...
	trans.Begin(ctx)
	b3, err = infs.OpenBtree[PersonKey, Person](ctx, tableName1, trans, Compare)
	if err != nil {
...
func Test_TwoTransactionsWithNoConflict(t *testing.T) {
	...
	trans2, _ := infs.NewTransaction(ctx, to)
	...
	b32, err := infs.OpenBtree[PersonKey, Person](ctx, tableName1, trans2, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.FailNow()
	}
...
func Test_VolumeAddThenSearch(t *testing.T) {
	...
	for i := start; i <= end; i++ {
		...
		if i%batchSize == 0 {
			...
			t1, _ = infs.NewTransaction(ctx, to)
			t1.Begin(ctx)
			b3, _ = infs.OpenBtree[PersonKey, Person](ctx, tableName1, t1, Compare)
		}
	}
...
func Test_TwoPhaseCommitRolledback(t *testing.T) {
	...
	b3, _ = infs.OpenBtree[int, string](ctx, tableName2, t1, nil)

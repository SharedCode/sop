...
func Test_ConcurrentKeyUpdates_DifferentItems(t *testing.T) {
	...
	b3, err := inredfs.NewBtree[PersonKeyWithMeta, Person](ctx, sop.StoreOptions{
		Name:                     tableName,
	...
	b3_t2, _ := inredfs.OpenBtree[PersonKeyWithMeta, Person](ctx, tableName, t2, ComparePersonKeyWithMeta)
	b3_t3, _ := inredfs.OpenBtree[PersonKeyWithMeta, Person](ctx, tableName, t3, ComparePersonKeyWithMeta)
	...
	t4, _ := inredfs.NewTransaction(ctx, to)
	t4.Begin(ctx)
	b3_t4, _ := inredfs.OpenBtree[PersonKeyWithMeta, Person](ctx, tableName, t4, ComparePersonKeyWithMeta)

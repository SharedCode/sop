...
func Test_TwoTransactionsWithNoConflict(t *testing.T) {
	...
	trans2, err := inredfs.NewTransaction(ctx, to)
	if err != nil {
		t.Fatal(err.Error())
	}
	...
	b32, err := inredfs.OpenBtree[PersonKey, Person](ctx, "persondb", trans2, Compare)
	if err != nil {
		t.Errorf("Error instantiating Btree, details: %v.", err)
		t.Fail()
	}
...
func Test_AddAndSearchManyPersons(t *testing.T) {
	...
	trans, err = inredfs.NewTransaction(ctx, tor)
	if err != nil {
		t.Error(err.Error())
		t.Fail()
		return

...
func Test_ConcurrentTransactions(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		go func(id int) {
			...
			store, err := inredfs.NewBtreeWithReplication[string, string](ctx, sop.StoreOptions{
				Name:       storeName,
				SlotLength: 10,
	...
	for i := 0; i < numGoroutines; i++ {
		...
		if i%2 == 0 {
			...
			_, err := inredfs.OpenBtreeWithReplication[string, string](ctx, storeName, verifyTx, cmp.Compare[string])
			if err != nil {

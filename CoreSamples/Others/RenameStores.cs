...
namespace Sop.Samples
{
    public class RenameStore
    {
        ...
        public void Run()
        {
            Console.WriteLine("{0}: RenameStore demo started...", DateTime.Now);
            ...
            using (var Server = new ObjectServer("SopBin\\OServer.dta"))
            {
                ...
                for (int i = 0; i < CollCount; i++)
                {
                    ...
                    if (i >= 10)
                    {
                        ...
                        store.Rename("foo12)");     // + (i-5).ToString());
                        //store.Delete();
                        //store.Remove();
                    }
                    Server.CycleTransaction();
                ...
                if (readStore)
                {
                    //var fooStore = Server.StoreNavigator.GetStore<long, Person>("SystemFile/foo", new StoreParameters<long> { CreateStoreIfNotExist = false });
                    for (int i = 0; i < CollCount; i++)
                    {
                        string CollectionName = string.Format("SystemFile/People{0}", i);
            ...
            Console.WriteLine("{0}: RenameStore demo ended...", DateTime.Now);
        }

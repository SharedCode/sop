...
There are four primary ingredients affecting performance and I/O via SOP. They are:
  ...
  * Cassandra Consistency level - specifying what consistency to use per API call can optimize/fine tune your data mgmgt. ACID transaction feature totally gives you a new dimension to address consistency, and the other attributes of ACID. see ConsistencyBook discussion above for details about consistency on API calls that can be configured.

...
"in_red_cfs" is complete and ready for production but it is NOT the recommended package because of Cassandra dependency. Replication is provided by C* and the ReedSolomon Erasure Coding (EC) based replication on the file node & large data (if your setup has any) blobs. So, that is fully safe in Production usage.
However, it is not as lean and as performance as the "in_red_fs" which still has replication on registry being completed.

Anyway, "in_red_fs" is really turning out to be a breakthrough tech's reference implementation. It is showing real cutting edge performance based on the SOP's new "swarm computing" and new "storage & caching strategy" that had been labelled as "revolutionary and a blue print of high performance, scalable storage system in a complex distributed environment", per AI (name withheld to protect AI's privacy).

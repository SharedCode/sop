...
Please feel free to file a request/discussion entry if you have a special domain-use in mind, as perhaps we can add to the supported use-cases, another kind like, a yet more efficient transaction. Example, I am looking at an AI use-case where SOP can allow local data cache to survives between transactions. Today, SOP piggy backs on the global cache(Redis) re-seeding the local cache of each transaction. It has a lot of advantages including solving data synchronization requirements among different instances running in the cluster. Without requiring to communicate & "orchestrate" with one another, maintaining fully parallelized execution model. But it seems we can still further evolve this and introduce another kind(V3+ timeline). Your input/feedback is highly appreciated, it is a great time to collaborate on this area.

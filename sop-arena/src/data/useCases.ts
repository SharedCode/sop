...
export const USE_CASES: UseCaseCard[] = [
  {
    ...
    sopAdvantage: 'Joltrin combines vector indexing, B-Tree persistent object storage, and swarm compute into a single embedded engine. Agent memory commits atomically, and worker failover redistributes reasoning tasks instantly.',
    metrics: [
  ...
  },
  {
    ...
    sopAdvantage: 'Joltrin provides embedded ordered B-Tree storage directly in the game server process. Transactions are serialized in microseconds, and server crashes failover without state rollbacks.',
    metrics: [
  ...
  },
  {
    ...
    sopAdvantage: 'Joltrin executes transactions locally using Write-Ahead Logging (WAL) and Snapshot Isolation. Invariant checks (sum balance == zero delta) verify prior to commit with zero network overhead.',
    metrics: [
  ...
  },
  {
    ...
    sopAdvantage: 'Joltrin compiles to standalone binaries and WebAssembly. Devices store, query, and transact locally with full ACID compliance, syncing to the swarm when connectivity resumes.',
    metrics: [
  ...
  },
  {
    ...
    sopAdvantage: 'Joltrin embeds directly inside the worker runtime. State is persisted to object storage or local NVMe with local caching, eliminating connection pooling bottlenecks entirely.',
    metrics: [

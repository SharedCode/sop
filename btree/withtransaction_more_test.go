...
func (f *fakeNRWithErr[TK, TV]) Get(ctx context.Context, id sop.UUID) (*Node[TK, TV], error) {
    if f.errOnGet { return nil, errors.New("nr get error") }
...
func (iatUpdateErr[TK,TV]) Update(ctx context.Context, item *Item[TK,TV]) error { return errors.New("update error") }
...
func (iatRemoveErr[TK,TV]) Remove(ctx context.Context, item *Item[TK,TV]) error { return errors.New("remove error") }
...
func TestWithTransaction_WritePreconditions(t *testing.T) {
    ...
    if _, err := w.Add(context.Background(), 1, "a"); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Add not-begun: %v", err) }
    if tx.rollbackCount != 0 { t.Fatalf("Add not-begun should not rollback") }
    if _, err := w.AddIfNotExist(context.Background(), 1, "a"); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("AddIfNotExist not-begun: %v", err) }
    if _, err := w.Upsert(context.Background(), 1, "a"); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Upsert not-begun: %v", err) }
    if _, err := w.Update(context.Background(), 1, "b"); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Update not-begun: %v", err) }
    if _, err := w.Remove(context.Background(), 1); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Remove not-begun: %v", err) }
    if _, err := w.RemoveCurrentItem(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("RemoveCurrentItem not-begun: %v", err) }

    // Read ops: not-begun must call Rollback and return error
    if _, err := w.Find(context.Background(), 1, true); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Find not-begun: %v", err) }
    if _, err := w.FindWithID(context.Background(), 1, sop.NilUUID); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("FindWithID not-begun: %v", err) }
    if _, err := w.GetCurrentValue(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("GetCurrentValue not-begun: %v", err) }
    if _, err := w.GetCurrentItem(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("GetCurrentItem not-begun: %v", err) }
    if _, err := w.First(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("First not-begun: %v", err) }
    if _, err := w.Last(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Last not-begun: %v", err) }
    if _, err := w.Next(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Next not-begun: %v", err) }
    if _, err := w.Previous(context.Background()); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("Previous not-begun: %v", err) }
    if tx.rollbackCount == 0 { t.Fatalf("read ops not-begun should rollback") }
}
...
func TestWithTransaction_NonWriterRollbackAndDelegatedErrors(t *testing.T) {
    ...
    for i, f := range calls {
        if err := f(); err == nil { t.Fatalf("expected error on non-writer call %d", i) }
    }
    if tx.rollbackCount != len(calls) { t.Fatalf("expected %d rollbacks for non-writer writes, got %d", len(calls), tx.rollbackCount) }
    ...
    if _, err := w2.Add(context.Background(), 1, "x"); err == nil { t.Fatalf("expected delegated Add error") }
    if tx2.rollbackCount != 1 { t.Fatalf("expected rollback on delegated Add error") }
    ...
    if _, err := w2b.AddIfNotExist(context.Background(), 2, "y"); err == nil { t.Fatalf("expected delegated AddIfNotExist error") }
    if tx2b.rollbackCount != 1 { t.Fatalf("expected rollback on delegated AddIfNotExist error") }
    ...
    if ok, err := b3.Add(context.Background(), 10, "v"); !ok || err != nil { t.Fatalf("seed add: %v", err) }
    w3 := NewBtreeWithTransaction[int, string](tx3, b3)
    if _, err := w3.Update(context.Background(), 10, "v2"); err == nil { t.Fatalf("expected delegated Update error") }
    if tx3.rollbackCount != 1 { t.Fatalf("expected rollback on delegated Update error") }
    ...
    if _, err := w4.Remove(context.Background(), 999); err == nil { t.Fatalf("expected delegated Remove error from Find") }
    if tx4.rollbackCount != 1 { t.Fatalf("expected rollback on delegated Remove error") }
    ...
    if _, err := w5.RemoveCurrentItem(context.Background()); err == nil { t.Fatalf("expected delegated RemoveCurrentItem error") }
    if tx5.rollbackCount != 1 { t.Fatalf("expected rollback on delegated RemoveCurrentItem error") }
}
...
func TestWithTransaction_ReadDelegatedErrorsAndGetCurrentKey(t *testing.T) {
    ...
    tx := &mockTx{begun: true, mode: sop.ForReading}
    w := NewBtreeWithTransaction[int, string](tx, b)
    if _, err := w.Find(context.Background(), 5, true); err == nil { t.Fatalf("expected delegated Find error") }
    if tx.rollbackCount != 1 { t.Fatalf("expected rollback on Find error") }
    ...
    it := w2.GetCurrentKey()
    if it.ID != sop.NilUUID || it.Key != 0 { t.Fatalf("expected zero current key when tx not begun") }
    ...
    if _, err := w3.GetCurrentItem(context.Background()); err == nil { t.Fatalf("expected delegated GetCurrentItem error") }
    if tx3.rollbackCount != 1 { t.Fatalf("expected rollback on GetCurrentItem error") }
}
...
func TestWithTransaction_UpdateCurrentItem_ModesAndError(t *testing.T) {
    ...
    if _, err := w1.UpdateCurrentItem(context.Background(), "x"); !errors.Is(err, errTransHasNotBegunMsg) { t.Fatalf("not-begun expected err, got %v", err) }

    // Non-writer
    tx2 := &mockTx{begun: true, mode: sop.ForReading}
    w2 := NewBtreeWithTransaction[int, string](tx2, b)
    if _, err := w2.UpdateCurrentItem(context.Background(), "x"); err == nil { t.Fatalf("expected error on non-writer UpdateCurrentItem") }
    if tx2.rollbackCount != 1 { t.Fatalf("expected rollback on non-writer UpdateCurrentItem") }
    ...
    tx3 := &mockTx{begun: true, mode: sop.ForWriting}
    w3 := NewBtreeWithTransaction[int, string](tx3, b3)
    if _, err := w3.UpdateCurrentItem(context.Background(), "x"); err == nil { t.Fatalf("expected delegated UpdateCurrentItem error") }
    if tx3.rollbackCount != 1 { t.Fatalf("expected rollback on delegated UpdateCurrentItem error") }
}

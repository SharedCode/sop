...
func TestAddItem_CoversDistributeAndPromote(t *testing.T) {
    ...
    for _, k := range vals {
        ...
        if err != nil || !ok { t.Fatalf("AddItem %d err=%v ok=%v", k, err, ok) }
    }
    if got := b.Count(); got != int64(len(vals)) { t.Fatalf("count=%d", got) }
    // Ensure tree has grown to have children at root (promote happened)
    rn, _ := b.getNode(nil, b.StoreInfo.RootNodeID)
    if !rn.hasChildren() { t.Fatalf("expected root to have children after AddItem operations") }
}

...
func (f *fakeNRSelectiveErr[TK, TV]) Get(_ context.Context, id sop.UUID) (*Node[TK, TV], error) {
    if f.errs[id] { return nil, fmt.Errorf("forced get error for %v", id) }
    return f.n[id], nil
}
...
func TestDistributeToRight_ReturnsParentError(t *testing.T) {
    ...
    v := "v"; vv := v
    for i := 0; i < b.getSlotLength(); i++ { node.Slots[i] = &Item[int, string]{Key: 100+i, Value: &vv, ID: sop.NewUUID()} }
    ...
    if err == nil { t.Fatalf("expected error from getParent to be returned") }
}
...
func TestDistributeToRight_ReturnsRightSiblingError(t *testing.T) {
    ...
    err := node.distributeToRight(nil, b, &Item[int, string]{Key: 999, Value: &vv, ID: sop.NewUUID()})
    if err == nil { t.Fatalf("expected error from getRightSibling to be returned") }
}
...
func TestDistribute_Controller_ReturnsError(t *testing.T) {
    ...
    v := "v"; vv := v
    for i := 0; i < b.getSlotLength(); i++ { left.Slots[i] = &Item[int, string]{Key: 10+i, Value: &vv, ID: sop.NewUUID()} }

...
func TestPrevious_AtFirstItem_ReturnsFalse(t *testing.T) {
    ...
    for _, k := range []int{2, 1, 3} { ok, _ := b.Add(nil, k, "v"); if !ok { t.Fatal("seed add") } }
    if ok, _ := b.First(nil); !ok { t.Fatal("First") }
    if ok, err := b.Previous(nil); err != nil || ok { t.Fatalf("Previous at first should be false,nil; got ok=%v err=%v", ok, err) }
    if b.isCurrentItemSelected() { t.Fatalf("selection should be cleared at start boundary") }
}
...
func TestPrevious_DescendLeftSubtree(t *testing.T) {
    ...
    left := newNode[int, string](b.getSlotLength()); left.newID(p.ID)
    mid := newNode[int, string](b.getSlotLength()); mid.newID(p.ID)
    right := newNode[int, string](b.getSlotLength()); right.newID(p.ID)
    p.ChildrenIDs[0] = left.ID; p.ChildrenIDs[1] = mid.ID; p.ChildrenIDs[2] = right.ID
    left.Slots[0] = &Item[int, string]{Key: 10, Value: &vv, ID: sop.NewUUID()}
    left.Slots[1] = &Item[int, string]{Key: 20, Value: &vv, ID: sop.NewUUID()}
    left.Count = 2
    fnr.Add(p); fnr.Add(left); fnr.Add(mid); fnr.Add(right)
    ...
    if ok, err := b.Previous(nil); err != nil || !ok { t.Fatalf("Previous err=%v ok=%v", err, ok) }
    if it, _ := b.GetCurrentItem(nil); it.Key != 20 { t.Fatalf("want 20, got %d", it.Key) }
}
...
func TestUnlinkNodeWithNilChild_EarlyReturn(t *testing.T) {
    ...
    fnr.Add(n)
    if ok, err := n.unlinkNodeWithNilChild(nil, b); err != nil || ok { t.Fatalf("expected false,nil early return; got ok=%v err=%v", ok, err) }
}

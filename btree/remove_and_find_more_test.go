...
func TestRemoveCurrentItem_InternalReplaceThenLeafDelete(t *testing.T) {
    ...
    if ok, _ := b.Find(nil, 30, false); !ok { t.Fatal("find 30") }
    ...
    if ok, err := b.RemoveCurrentItem(nil); err != nil || !ok { t.Fatalf("RemoveCurrentItem err=%v ok=%v", err, ok) }
    if got := b.Count(); got != 4 { t.Fatalf("count after remove=%d", got) }
    // Sanity: key 30 should no longer be present
    if ok, _ := b.Find(nil, 30, false); ok { t.Fatalf("30 should be removed") }
...
func TestFindWithID_DuplicateNonFirst(t *testing.T) {
    ...
    root := newNode[int, string](b.getSlotLength()); root.newID(sop.NilUUID); b.StoreInfo.RootNodeID = root.ID; fnr.Add(root)

    // Add duplicates for key 5
    ids := []sop.UUID{}
    for i := 0; i < 3; i++ {
        v := "v"; vv := v
        id := sop.NewUUID(); ids = append(ids, id)
        if ok, err := b.AddItem(nil, &Item[int, string]{Key: 5, Value: &vv, ID: id}); err != nil || !ok { t.Fatalf("add dup %d", i) }
    }
    // FindWithID should walk Next until matching id
    if ok, err := b.FindWithID(nil, 5, ids[2]); err != nil || !ok { t.Fatalf("FindWithID err=%v ok=%v", err, ok) }
    it, _ := b.GetCurrentItem(nil)
    if it.ID != ids[2] { t.Fatalf("expected id match on third duplicate") }
}
...
func TestCurrent_Fetch_NoSelection(t *testing.T) {
    ...
    if _, err := b.GetCurrentValue(nil); err != nil { t.Fatalf("GetCurrentValue unexpected err: %v", err) }
    if _, err := b.GetCurrentItem(nil); err != nil { t.Fatalf("GetCurrentItem unexpected err: %v", err) }
}

...
func TestCurrent_Fetch_RepoError(t *testing.T) {
    ...
    if _, err := b.GetCurrentItem(context.Background()); err == nil { t.Fatalf("expected repo get error in GetCurrentItem") }
    if _, err := b.GetCurrentValue(context.Background()); err == nil { t.Fatalf("expected repo get error in GetCurrentValue") }
}

...
func TestAddOnLeaf_NotFull_InsertsAndShifts(t *testing.T) {
    ...
    if err := leaf.addOnLeaf(nil, b, item, 1); err != nil { t.Fatalf("addOnLeaf err: %v", err) }
    if leaf.Count != 3 || leaf.Slots[1] == nil || leaf.Slots[1].Key != 20 { t.Fatalf("expected insert+shift; got count=%d mid=%v", leaf.Count, leaf.Slots[1]) }
}
...
func TestAddOnLeaf_Full_VacancyLeft_SetsDistributeLeft(t *testing.T) {
    ...
    left := newNode[int,string](b.getSlotLength()); left.newID(parent.ID)
    // left has room
    v := "v"; vv := v
    left.Slots[0] = &Item[int,string]{Key: 1, Value: &vv, ID: sop.NewUUID()}
    left.Count = 1

    cur := newNode[int,string](b.getSlotLength()); cur.newID(parent.ID)
    fillFullNode(b, cur, 10)

    right := newNode[int,string](b.getSlotLength()); right.newID(parent.ID)
    ...
    fnr.Add(parent); fnr.Add(left); fnr.Add(cur); fnr.Add(right)

    item := &Item[int,string]{Key: 15, Value: &vv, ID: sop.NewUUID()}
    if err := cur.addOnLeaf(nil, b, item, 1); err != nil { t.Fatalf("addOnLeaf err: %v", err) }
    if b.distributeAction.sourceNode != cur || !b.distributeAction.distributeToLeft || b.distributeAction.item == nil { t.Fatalf("expected distribute left to be set") }
}
...
func TestAddOnLeaf_Full_VacancyRight_SetsDistributeRight(t *testing.T) {
    ...
    item := &Item[int,string]{Key: 15, Value: &vv, ID: sop.NewUUID()}
    if err := cur.addOnLeaf(nil, b, item, 1); err != nil { t.Fatalf("addOnLeaf err: %v", err) }
    if b.distributeAction.sourceNode != cur || b.distributeAction.distributeToLeft || b.distributeAction.item == nil { t.Fatalf("expected distribute right to be set") }
}
...
func TestAddOnLeaf_Full_Unbalanced_SplitsIntoChildren(t *testing.T) {
    ...
    fnr.Add(parent); fnr.Add(right); fnr.Add(cur)
...
func TestAddOnLeaf_Full_AllFull_PromoteRequested(t *testing.T) {
    ...
    if b.promoteAction.targetNode == nil || b.promoteAction.targetNode.ID != parent.ID { t.Fatalf("expected promote action to target parent") }
}
...
func TestAddOnLeaf_Full_RootBreaksIntoChildren(t *testing.T) {
    ...
    v := "v"; vv := v
    item := &Item[int,string]{Key: 15, Value: &vv, ID: sop.NewUUID()}
    if err := root.addOnLeaf(nil, b, item, 1); err != nil { t.Fatalf("addOnLeaf err: %v", err) }

    if root.ChildrenIDs == nil || root.Count != 1 { t.Fatalf("expected root to split into two children; count=%d children=%v", root.Count, root.ChildrenIDs) }
}

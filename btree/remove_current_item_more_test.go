...
func TestRemoveCurrentItem_WithChildren_LeftNilChild_Shortcut(t *testing.T) {
    ...
    if err != nil || !ok { t.Fatalf("RemoveCurrentItem err=%v ok=%v", err, ok) }
    if b.isCurrentItemSelected() { t.Fatalf("selection should be cleared") }
    if b.Count() != 1 { t.Fatalf("Store count should decrement to 1, got %d", b.Count()) }
}
...
func TestRemoveCurrentItem_RootSingleItem_Delete(t *testing.T) {
    ...
    ok, err := b.RemoveCurrentItem(context.TODO())
    if err != nil || !ok { t.Fatalf("RemoveCurrentItem root err=%v ok=%v", err, ok) }
    if b.isCurrentItemSelected() { t.Fatalf("selection should be cleared") }
    if b.Count() != 0 { t.Fatalf("store count should be 0, got %d", b.Count()) }
    if root.Count != 0 || root.Slots[0] != nil { t.Fatalf("root not cleared") }
}
...
func TestRemoveCurrentItem_NonRoot_UnlinkEarly(t *testing.T) {
    ...
    child.Count = 1
    fnr.Add(parent); fnr.Add(child)
    b.StoreInfo.RootNodeID = parent.ID
    b.StoreInfo.Count = 1
    b.setCurrentItemID(child.ID, 0)

    ok, err := b.RemoveCurrentItem(context.TODO())
    if err != nil || !ok { t.Fatalf("RemoveCurrentItem unlink err=%v ok=%v", err, ok) }
    if b.isCurrentItemSelected() { t.Fatalf("selection should be cleared") }
    if b.Count() != 0 { t.Fatalf("store count should be 0, got %d", b.Count()) }
}

...
func TestFileIOSimulator_ErrorInjectionAndResetScenarios(t *testing.T) {
    ...
    if err := sim.WriteFile(ctx, "file_1", []byte("x"), 0o644); err == nil { t.Fatalf("expected write error") }
    // Write different suffix ok
    if err := sim.WriteFile(ctx, "file_2", []byte("ok"), 0o644); err != nil { t.Fatalf("write ok: %v", err) }
    if _, err := sim.ReadFile(ctx, "file_2"); err != nil { t.Fatalf("initial read: %v", err) }
    // Induce read error w/ auto-reset on suffix _2 (second flag)
    sim.setResetFlag(true)
    sim.setErrorOnSuffixNumber2(2)
    if _, err := sim.ReadFile(ctx, "file_2"); err == nil { t.Fatalf("expected induced read error") }
    if b, err := sim.ReadFile(ctx, "file_2"); err != nil || string(b) != "ok" { t.Fatalf("post-reset read: %v %q", err, string(b)) }
    // Remove error injection via write flag then normal remove
    sim.setErrorOnSuffixNumber(2)
    if err := sim.Remove(ctx, "file_2"); err == nil { t.Fatalf("expected induced remove err") }
    sim.setErrorOnSuffixNumber(-1)
    if err := sim.Remove(ctx, "file_2"); err != nil { t.Fatalf("remove: %v", err) }
    // Reset flag path using suffix _0 via second flag
    name := "dummy_0"
    if err := sim.WriteFile(ctx, name, []byte("ok"), 0o644); err != nil { t.Fatalf("seed: %v", err) }
    sim.setResetFlag(true); sim.setErrorOnSuffixNumber2(0)
    if _, err := sim.ReadFile(ctx, name); err == nil { t.Fatalf("expected read err for suffix 0") }
    if b, err := sim.ReadFile(ctx, name); err != nil || string(b) != "ok" { t.Fatalf("second read expected ok: %v %q", err, string(b)) }
}
...
func TestManageStoreFolder_CreateRemove(t *testing.T) {
    ...
    ms := NewManageStoreFolder(nil)
    if err := ms.CreateStore(ctx, base); err != nil { t.Fatalf("CreateStore: %v", err) }
    if _, err := os.Stat(base); err != nil { t.Fatalf("expected folder exists: %v", err) }
    if err := ms.RemoveStore(ctx, filepath.Dir(base)); err != nil { t.Fatalf("RemoveStore: %v", err) }
}
...
func TestRetryIO_RetryableThenSuccess(t *testing.T) {
    ...
    err := retryIO(ctx, func(context.Context) error {
        ...
        if a <= 2 { return sop.Error{Code: sop.FileIOError, Err: errors.New("transient") } }
        return nil
    })
    if err != nil { t.Fatalf("retryIO unexpected error: %v", err) }
...
func TestApply4LevelHierarchy(t *testing.T) {
    ...
    if len(parts) != 4 { t.Fatalf("expected 4 parts, got %d: %q", len(parts), got) }
    if parts[0] != "a" || parts[1] != "b" || parts[2] != "c" || parts[3] != "d" { t.Fatalf("expected a/b/c/d, got %q", got) }
}
func TestDefaultToFilePath_Variants(t *testing.T) {
    ...
    for _, base := range cases {
        ...
        if !strings.Contains(got, sep+"a"+sep+"b"+sep+"c"+sep+"d") { t.Fatalf("missing hierarchy in %q", got) }
    }
}
func TestToFilePath_OverrideAndRestore(t *testing.T) {
    old := ToFilePath; t.Cleanup(func(){ ToFilePath = old })
    id := sop.NewUUID()
    ToFilePath = func(base string, id sop.UUID) string { return "X:"+base+":"+id.String() }
    if !strings.HasPrefix(ToFilePath("base", id), "X:base:") { t.Fatalf("override not effective") }
}

// Documentation-only unreachable mismatch branches (from registrymap_mismatch_unreachable_cases_test.go).
func TestRegistryMapMismatchUnreachable(t *testing.T) { t.Skip("registryMap mismatch branches currently unreachable; see hashing logic") }

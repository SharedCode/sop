...
func TestBlobStoreWithEC_GetOneMissingConfig(t *testing.T) {
    ...
    if _, e := bs.(*blobStoreWithEC).GetOne(ctx, "absent", sop.NewUUID()); e == nil { t.Fatalf("expected missing config error for GetOne") }
}
...
func TestBlobStoreWithEC_AddMissingConfig(t *testing.T) {
    ...
    if err != nil { t.Fatalf("setup: %v", err) }
    id := sop.NewUUID()
    if e := bs.Add(ctx, []sop.BlobsPayload[sop.KeyValuePair[sop.UUID, []byte]]{{BlobTable: "abs", Blobs: []sop.KeyValuePair[sop.UUID, []byte]{{Key: id, Value: []byte("x")}}}}); e == nil { t.Fatalf("expected missing config error for Add") }
}
...
func TestBlobStoreWithEC_RemoveMissingConfig(t *testing.T) {
    ...
    bs, err := NewBlobStoreWithEC(nil, nil, cfg)
    if err != nil { t.Fatalf("setup: %v", err) }
    if e := bs.Remove(ctx, []sop.BlobsPayload[sop.UUID]{{BlobTable: "zzz", Blobs: []sop.UUID{sop.NewUUID()}}}); e == nil { t.Fatalf("expected missing config error for Remove") }
}

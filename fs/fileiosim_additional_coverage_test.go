...
func TestFileIOSim_ReadNotFoundAndResetFalse(t *testing.T) {
    ...
    if _, err := sim.ReadFile(ctx, "foo_5"); err == nil { t.Fatalf("expected induced error") }
    // second read should still error because resetFlag(false)
    if _, err := sim.ReadFile(ctx, "foo_5"); err == nil { t.Fatalf("expected induced error persist") }
}

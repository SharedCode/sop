...
func (s *stubECFileIO) ReadFile(ctx context.Context, name string) ([]byte, error) { return nil, errors.New("read not implemented") }
...
func (r readErrFileIO) WriteFile(context.Context, string, []byte, os.FileMode) error { return errors.New("no writes") }
func (r readErrFileIO) ReadFile(context.Context, string) ([]byte, error)            { return nil, errors.New("induced read fail") }
...
func testECConfig(table string, data, parity int) map[string]ErasureCodingConfig {
    ...
    for i := 0; i < total; i++ { base[i] = "d" + string(rune('0'+i)) }
...
func TestBlobStoreWithEC_Add_FailureToleranceAndExceed(t *testing.T) {
    ...
    if err := bs1.Add(ctx, payload); err != nil { t.Fatalf("expected tolerated add, got %v", err) }
    if fio1.writeCalls != 2 { t.Fatalf("expected 2 shard writes got %d", fio1.writeCalls) }

    // Exceed parity failures
    fio2 := &stubECFileIO{failAll: true}
    bs2, _ := NewBlobStoreWithEC(DefaultToFilePath, fio2, cfg)
    if err := bs2.Add(ctx, payload); err == nil { t.Fatalf("expected error exceeding parity tolerance") }
}
...
func TestBlobStoreWithEC_GetOne_AllReadsFail(t *testing.T) {
    ...
    if err != nil { t.Fatalf("NewBlobStoreWithEC: %v", err) }
    if _, err := bs.GetOne(ctx, table, sop.NewUUID()); err == nil { t.Fatalf("expected read failure error") }
}

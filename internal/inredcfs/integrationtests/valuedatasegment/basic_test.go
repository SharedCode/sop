...
func Test_TransactionStory_OpenVsNewBTree(t *testing.T) {
	...
	if _, err := inredcfs.OpenBtree[int, string](ctx, "fooStore22", trans, nil); err == nil {
...
func Test_ByteArrayValue(t *testing.T) {
	inredcfs.RemoveBtree(ctx, "baStore")
	trans, err := inredcfs.NewTransaction(sop.ForWriting, -1, false)
	if err != nil {
		t.Fatal(err.Error())
	}
...
func Test_ByteArrayValueGet(t *testing.T) {
	trans, err := inredcfs.NewTransaction(sop.NoCheck, -1, false)
	if err != nil {
		t.Fatal(err.Error())
	}
	trans.Begin()
